 ...
 */
function getHeaderComment(context, eol) {
  const comments = context.sourceCode.getAllComments();
  const startingIndex = comments[0].type === "Shebang" ? 1 : 0;

  if (comments[startingIndex].type === "Block") {
    return {
      value: comments[startingIndex].value,
      loc: comments[startingIndex].loc,
      range: comments[startingIndex].range,
      type: "Block",
    };
  }
  ...
  return {
    value: lineComments.map((comment) => comment.value).join(eol),
    range: [
  ...
  create(context) {
    ...
    return {
      Program: function (node) {
        ...
        const headerComment = getHeaderComment(context, eol);
        const headerCommentLines = new CommentParser(formatConfig).parse(
          headerComment.value,
        );
        const headerCommentText = headerCommentLines.join(eol);

        const mismatchedBlockPrefix =
          formatConfig.blockPrefix &&
          !headerComment.value.startsWith(formatConfig.blockPrefix);
        const mismatchedBlockSuffix =
          formatConfig.blockSuffix &&
          !headerComment.value.endsWith(formatConfig.blockSuffix);

        if (
          !headerCommentText.startsWith(expectedHeaderText) ||
          mismatchedBlockPrefix ||
          mismatchedBlockSuffix
        ) {
          const headerPragmas = headerCommentLines.filter((line) =>
            line.startsWith("@"),
          );
